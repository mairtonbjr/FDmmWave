function varargout = lin2dbm(varargin)
% DESCRIPTION y = lin2dbm(x1, x2, ...)
%  Converts linear to dBm, no warning if input is zero.
...
%  x3  -- Real, ...
% OUTPUT
%  y1 -- As 1st input, but coverted into dBm.
%  y2 -- As 2nd input, but coverted into dBm.
%  y3 -- As 3rd input, but ...
% TRY
...
warning off
varargout{i} = 10*log10(varargin{i}+0)+30; % +0 enabler for operation on logical variables
warning on
end
