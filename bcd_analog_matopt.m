function [bcd_mat_analog] = bcd_analog_matopt(quant_set,mat_analog,mat_A,mat_C, mat_B)
...
while iter_inner_BCD <= mat_iter
    ...
    for idxRow = 1:size(mat_analog,1)
        for idxCol = 1:size(mat_analog,2)
            ...
            % problem
            obj_scal_analog = real(scal_b'*quant_set);
            [~,idx_opt] = max(obj_scal_analog);
            % Select the best one and assign it to update the DL analog combiner
            mat_elem = quant_set(idx_opt);
